...
query = """
SELECT cluster, COUNT(DISTINCT PersonID) AS ClaimantsCount, SUM(ObesityCost) AS TotalCostPerCluster,
SUM(ObesityCost)/COUNT(DISTINCT PersonID) AS CostPerClaimant
FROM COSTS
GROUP BY Cluster
ORDER BY Cluster
"""
...
query = """
SELECT FinancialYear, Cluster, COUNT(DISTINCT PersonID) AS ClaimantsCount, SUM(ObesityCost) AS TotalCostPerCluster,
SUM(ObesityCost)/COUNT(DISTINCT PersonID) AS CostPerClaimant
FROM COSTS
