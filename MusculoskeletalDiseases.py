...
query = """
SELECT cluster, COUNT(DISTINCT PersonID) AS ClaimantsCount, SUM(MusculoskeletalDiseasesCost) AS TotalCostPerCluster,
SUM(MusculoskeletalDiseasesCost)/COUNT(DISTINCT PersonID) AS CostPerClaimant
FROM COSTS
GROUP BY Cluster
ORDER BY Cluster
"""
...
query = """
SELECT FinancialYear, Cluster, COUNT(DISTINCT PersonID) AS ClaimantsCount, SUM(MusculoskeletalDiseasesCost) AS TotalCostPerCluster,
SUM(MusculoskeletalDiseasesCost)/COUNT(DISTINCT PersonID) AS CostPerClaimant
FROM COSTS
...
plot_me = plot_me[plot_me.FinancialYear != 1902]
plot_me.head()
